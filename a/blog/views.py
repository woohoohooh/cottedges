...
def post_list(request):
    ...
    categories = TagPosts.objects.all()
    return render(request, 'blog/post_list.html', {'posts': posts, 'tags_posts': tags_posts, 'categories': categories, 'request': request})

def post_detail(request, pk):
    tags_posts = TagPosts.objects.all()
    post = get_object_or_404(Post, pk=pk)
    comments = post.comments.all()
    return render(request, 'blog/post_detail.html', {'post': post, 'comments': comments, 'tags_posts': tags_posts})
